...
def text_cleaner(df):  # Zhang, 2023
    # lower
    ...
    df["description"] = df["description"].str.replace(r" [wW] ", " with ")
    df["description"] = df["description"].str.replace(
        r"\s[bcdefghijklmnopqrstuvwxyzBCDEFGHIJKLMNOPQRSTUVWXYZ]\s", " "
    )
    df["description"] = df["description"].str.replace(
        r"\s[bcdefghijklmnopqrstuvwxyzBCDEFGHIJKLMNOPQRSTUVWXYZ]\s", " "
    )
    ...
    df["description"] = df["description"].str.replace(
        r"\s[bcdefghijklmnopqrstuvwxyzBCDEFGHIJKLMNOPQRSTUVWXYZ]$", " "
    )
    df["description"] = df["description"].str.replace(
        r"^[bcdefghijklmnopqrstuvwxyzBCDEFGHIJKLMNOPQRSTUVWXYZ]\s", " "
    )
    ...
    df["description"] = df["description"].str.replace(
        " eng hardwood ", " engineered hardwood "
    )
    ...
    df["description"] = df["description"].str.replace(
        " washer and dyer ", " washer and dryer "
    )
    ...
    df["description"] = df["description"].str.replace(
        "over the range microwave", "otr microwave"
    )
    df["description"] = df["description"].str.replace(
        "over range microwave", "otr microwave"
    )
    ...
    df["description"] = df["description"].str.replace("^super market ", " supermarket ")
    df["description"] = df["description"].str.replace(
        " etobicoke ci ", " etobicoke collegiate institute "
    )
...
def text_preprocess(df):
    df = text_cleaner(df)
    df = text_tokenizer(df)
    df["description"] = df["description"].apply(lambda x: numtoword(x))

    lemmatiser = WordNetLemmatizer()
    df["description"] = df["description"].apply(
        lambda x: [lemmatiser.lemmatize(str(word)) for word in x]
    )
