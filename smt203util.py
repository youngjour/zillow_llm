...
def calculate_log_odds_idp(
    global_counts, counts_i_name, counts_i_dict, counts_j_name, counts_j_dict
):
    """calculate log odds"""
    global_df = pd.DataFrame(
        list(global_counts.items()), columns=["word", "global_count"]
    )

    global_df[counts_i_name] = global_df.word.apply(
        lambda word: counts_i_dict[word] if word in counts_i_dict.keys() else 0
    )
    global_df[counts_j_name] = global_df.word.apply(
        lambda word: counts_j_dict[word] if word in counts_j_dict.keys() else 0
    )
    ...
    delta = (
        ((yiw + aw) / (ni + a0 - (yiw + aw))) / ((yjw + aw) / (nj + a0 - (yjw + aw)))
    ).apply(np.log)
...
def find_discriminative_words(
    top_words_df, threshold_i=5, threshold_j=5, num_i=100, num_j=100, mypath="."
):
    ...
    with open(f"{mypath}/word_counts/{counts_i_name}_zscore.csv", "w") as output:
    ...
    with open(f"{mypath}/word_counts/{counts_j_name}_zscore.csv", "w") as output:
        for index, row in tmp.iterrows():
            output.write(
                ",".join(
